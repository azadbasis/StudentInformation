...
public class StudentActivity extends AppCompatActivity {
    ...
    ListView resultListLv;
    AdapterForResultList adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        resultListLv = (ListView) findViewById(R.id.resultLv);
    ...
    private void onClick() {
        resultListLv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {

                Intent intent = new Intent(StudentActivity.this, ResultInformationActivity.class);
                int resultId = resultList.get(position).getResultId();
                intent.putExtra("resultId", resultId);
                intent.putExtra("bitForUpdate", 2);
                intent.putExtra("extraId", showKey);
    ...
    public void btnAddResult(View view) {
        Intent intent = new Intent(StudentActivity.this, ResultInformationActivity.class);
        intent.putExtra("extraId",showKey);
        startActivity(intent);
    ...
    private void getResultList() {
        ...
        resultList = manager.getAllResultByExtraId(String.valueOf(showKey));
        adapter = new AdapterForResultList(this, resultList);
        resultListLv.setAdapter(adapter);

        if (resultList.isEmpty()) {
            resultListLv.setVisibility(View.GONE);
        } else {
            resultListLv.setVisibility(View.VISIBLE);
        }
